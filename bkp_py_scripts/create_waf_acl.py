...
list_of_uris = [
    "/api/air/v1.0/connector/travelport", 
    "/api/air/v1.0/connector/mystifly",
    "/api/air/v1.0/connector/sabre"
    ]

rule_name = "FlightsConnetcorRule"
...
def create_uri_condition(waf_client):
    ...
    Name='Flights-Connector-URI-Conditions',
...
def create_ip_condition(waf_client):
    change_token = get_change_token(waf_client)
    response = waf_client.create_ip_set(
        Name='Flights-Connector-IP-Conditions',
        ChangeToken=change_token
    )
    condition_id = response.get("ByteMatchSet").get("ByteMatchSetId")
    return condition_id

def update_ip_condition(waf_client, condition_id):
    change_token = get_change_token(waf_client)
    waf_client.update_byte_match_set(
        ByteMatchSetId=condition_id,
        ChangeToken=change_token,
...
def main():
    ...
    uri_condition_id = create_uri_condition(waf_client)
    update_uri_condition(waf_client, uri_condition_id)
    ip_condition_id = create_ip_condition(waf_client)
    update_ip_condition(waf_client, ip_condition_id)
